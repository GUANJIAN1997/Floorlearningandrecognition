...
def getmaxbarometric():
    ...
    deleted_upordown_number2[43] = 43
    ...
    for i in range(len(deleted_upordown_number2)):              #1.5一个，编号，0到总个数，其中上下楼的编号已经被标注成"階層移動"
        if deleted_upordown_number2[i] != "階層移動":
            ...
            for i in jiao_7:
                for j in i:
                    if j not in jiao_7_number:
                        jiao_7_number.append(j)
            ...
            for i in range(len(jiao_7)):
                ...
                if jiao_7[i]:
                    for j in jiao_7[i]:
                        for ii in range(16):
                            if floor7_pro_dic[i][j][ii] > 0:
                                P_sum += learned_floor_dic[i][j][ii]
                    P_sum2 = round(P_sum  / len(learned_floor_dic[i]),2)
                    # P_sum2 = round(P_sum / len(jiao_7[i]), 2)
                    # P_sum2 = round(P_sum  / len(jiao_7_number), 2)
                    P_sum_list.append(P_sum2)
